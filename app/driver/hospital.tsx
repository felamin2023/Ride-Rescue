...
type PlaceRow = {
  place_id: string;
  name: string | null;
  category: "hospital" | (string & {});
  address: string | null;
  plus_code: string | null;
  latitude: string | number | null;
  longitude: string | number | null;
  maps_link: string | null;
  phone?: string | null; // requires optional phone column
};
...
/* --------------------------------- Filters -------------------------------- */
const FILTERS: FilterItem[] = [{ key: "hospital", icon: "medical-outline", label: "Hospital" }];
      ...
      <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.45)" }}>
        ...
        >
          ...
          <View className="flex-row items-center gap-3 pb-3">
            <View style={{ width: 44, height: 44, borderRadius: 999, overflow: "hidden", backgroundColor: "#F1F5F9" }}>
              {facility.avatar ? <RNImage source={{ uri: facility.avatar }} style={{ width: "100%", height: "100%" }} /> :
                <View className="h-full w-full items-center justify-center"><Ionicons name="medical-outline" size={20} color="#475569" /></View>}
            </View>
          ...
          <View className="mt-3 gap-3">
            ...
            <PrimaryButton label="Call Hospital" variant="secondary" icon="call-outline" onPress={() => { onCall(facility); onClose(); }} />
            <PrimaryButton
          ...
          <View className="rounded-2xl bg-white p-4" style={[{ borderWidth: 1, borderColor: COLORS.border }, panelShadow]}>
            <View className="flex-row items-start gap-3">
              <View style={{ width: 56, height: 56, borderRadius: 999, overflow: "hidden", backgroundColor: "#F1F5F9" }}>
                {facility.avatar ? <RNImage source={{ uri: facility.avatar }} style={{ width: "100%", height: "100%" }} /> :
                  <View className="h-full w-full items-center justify-center"><Ionicons name="medical-outline" size={22} color="#475569" /></View>}
              </View>
...
export default function HospitalScreen() {
  ...
  const callFacility = (s: Facility) => {
    if (!s.phone) return;
    Linking.openURL(`tel:${s.phone}`).catch(() => {});
  };
    ...
    <View className="flex-1" style={{ backgroundColor: COLORS.bg }}>
        ...
        <View className="relative px-4 py-3">
          ...
          {!searchOpen && (
            <View pointerEvents="none" style={{ position: "absolute", left: 0, right: 0, top: 12, alignItems: "center" }}>
              <Text className="text-xl font-bold text-[#0F172A]">Hospitals</Text>
            </View>
          )}
        </View>
      ...
      {(gotLocation === "denied" || gotLocation === "error") && (
        <View className="mx-4 mt-3 rounded-2xl bg-white p-3" style={[{ borderColor: COLORS.border, borderWidth: 1 }, panelShadow]}>
          <Text className="text-[12px] text-slate-600">We couldn’t access your location. Showing hospitals without distance.</Text>
        </View>
      )}

      {/* Filters */}
      <FilterChips items={FILTERS} selected={filters} onToggle={toggleFilter} containerStyle={{ paddingHorizontal: 16, marginTop: 10 }} gap={8} horizontal accessibilityLabel="Facility filters" />
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="mt-16 items-center">
            ...
            <Text className="mt-2 text-slate-500">No hospitals found.</Text>
          </View>
        }
      />
