...
/* --------------------------------- Filters (same order as vulcanize) -------------------------------- */
...
/* ----------------------- Bottom Sheet (identical to vulcanize) ---------------------- */
  ...
  return (
    ...
    >
      ...
      <View style={{ paddingTop: 8, paddingLeft: 68 }}>
        {/* Address first (13px + optional 12px) */}
        <Text className="text-[13px] text-slate-700" numberOfLines={2}>{shop.address1}</Text>
        {shop.address2 ? (
          <Text className="text-[12px] text-slate-500" numberOfLines={1}>{shop.address2}</Text>
        ) : null}
...
/* --------------------------------- Screen (header + inline search identical) --------------------------------- */
export default function GasStationScreen() {
  const router = useRouter();
  const [query, setQuery] = useState("");
  const [filters, setFilters] = useState<string[]>(["gas"]);
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: COLORS.bg }}>
      ...
      {/* Filter chips (match spacing/gap) */}
      <FilterChips
        items={FILTERS}
        selected={filters}
      ...
      {/* Details modal (present, not opened by default) */}
      <DetailsModal
        visible={detailsOpen}
        shop={selectedShop}
